    ...
    if (hotelId && !slugLoading && passwordChangeRequired === false) {
      ...
      getHotelSlugFromId(hotelId)
        ...
        .catch(err => {
          ...
          // Use the slug already in the URL if present; otherwise null → fall through to /login
          // (do NOT fall back to a hardcoded slug, which would 404 when that hotel doesn't exist)
          const urlSlug = window.location.pathname.match(/\/hotel\/([^\/]+)/)?.[1] ?? null
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-bliss-50">
        <div className="text-center">
          ...
          <p className="text-bliss-600">กำลังค้นหาข้อมูลโรงแรม...</p>
        </div>
      </div>
    )
  }
