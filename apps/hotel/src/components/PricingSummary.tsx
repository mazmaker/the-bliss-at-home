...
function PricingSummary({ configuration, showDetails = true, compact = false }: PricingSummaryProps) {
  ...
  if (selections.length === 0) {
    return (
      <div className="bg-gradient-to-br from-bliss-50 to-bliss-100 border-2 border-bliss-200 rounded-xl p-6 text-center shadow-sm">
        <Receipt className="w-10 h-10 text-bliss-400 mx-auto mb-3" />
        <p className="text-sm font-medium text-bliss-600">ยังไม่ได้เลือกบริการ</p>
      </div>
    )
  }
  ...
  return (
    <div className={`
      bg-white border-2 border-[#565b34]/30 rounded-xl overflow-hidden shadow-lg
      ${compact ? 'p-4' : 'p-6'}
    `}>
      {/* Header */}
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 bg-gradient-to-r from-[#565b34] to-[#565b34]/90 rounded-full flex items-center justify-center shadow-md">
          <Receipt className="w-5 h-5 text-white" />
        </div>
        <div>
          <h3 className={`font-semibold text-bliss-900 ${compact ? 'text-base' : 'text-lg'}`}>
            สรุปการจอง
          </h3>
          ...
          {Object.entries(getServicesByRecipient()).map(([recipientIndex, recipientServices]) => (
            <div key={recipientIndex} className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-[#7a875f] rounded-full flex items-center justify-center shadow-sm">
                  <User className="w-3.5 h-3.5 text-white" />
                </div>
                <span className="text-sm font-bold text-bliss-800">
              ...
              {recipientServices.map((selection) => (
                <div key={selection.id} className="ml-8 bg-gradient-to-r from-[#ebe6d0]/20 to-[#ebe6d0]/10 border border-[#ebe6d0]/30 rounded-lg p-3 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <h5 className="font-medium text-bliss-900 text-sm mb-1">
                        {selection.service.name_th}
                      </h5>
                      <div className="flex items-center gap-3 text-xs text-bliss-600">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-bold text-[#565b34]">
      ...
      {/* Summary section */}
      <div className="border-t border-bliss-100 pt-4 space-y-3">
        {/* Duration summary */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Clock className="w-4 h-4 text-bliss-600" />
            <span className="text-sm text-bliss-700">เวลารวม</span>
          </div>
        ...
        {mode === 'couple' && coupleFormat === 'simultaneous' ? (
          <>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-bliss-700">ผู้ให้บริการ 1</span>
              </div>
              <span className="text-sm font-medium text-bliss-900">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-bliss-700">ผู้ให้บริการ 2</span>
              </div>
              <span className="text-sm font-medium text-bliss-900">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-bliss-700">จำนวนผู้รับบริการ</span>
            </div>
            <span className="text-sm font-medium text-bliss-900">
              {recipientCount} ท่าน
            </span>
        ...
        {/* Services count */}
        <div className="flex items-center justify-between">
          <span className="text-sm text-bliss-700">จำนวนบริการ</span>
          <span className="text-sm font-medium text-bliss-900">
