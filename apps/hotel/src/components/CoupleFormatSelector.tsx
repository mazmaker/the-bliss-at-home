          ...
          return (
            <button
              ...
              className={`
                ...
                ${isSelected
                  ? 'border-[#565b34] bg-gradient-to-br from-[#ebe6d0] to-[#ebe6d0]/70 shadow-lg transform scale-[1.01]'
                  : 'border-bliss-300 bg-white hover:border-[#565b34] hover:bg-[#ebe6d0]/20 hover:shadow-md'
                }
                ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
                focus:outline-none focus:ring-2 focus:ring-[#565b34] focus:ring-offset-2
              `}
            >
              {/* Selection indicator */}
              {isSelected && (
              ...
              {/* Icon */}
              <div className={`
                inline-flex p-4 rounded-xl mb-4 ${isSelected ? 'bg-[#565b34]' : 'bg-gradient-to-r from-[#7a875f] to-[#7a875f]/80'}
                shadow-sm
              `}>
              ...
              <div className="space-y-4 flex-1 flex flex-col">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-bliss-700">
                    {format.subtitle}
                  </p>
                </div>

                {/* Description */}
                <p className="text-sm text-bliss-600 leading-relaxed">
                  {format.description}
                </p>
                ...
                <div className="space-y-2">
                  {format.features.map((feature, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-bliss-400 rounded-full flex-shrink-0" />
                      <span className="text-xs text-bliss-600">{feature}</span>
                    </div>
                  ))}
                </div>
