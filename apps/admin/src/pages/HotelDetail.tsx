...
export default function HotelDetail() {
  ...
  const getStatusBadge = (status: string) => {
    const badges = {
      ...
      inactive: { bg: 'bg-bliss-100', text: 'text-bliss-700', label: 'ไม่ใช้งาน', icon: XCircle },
    ...
    return (
      <div className="flex h-96 items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-bliss-600">กำลังโหลดข้อมูล...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex h-96 items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="mt-4 text-lg font-semibold text-bliss-900">เกิดข้อผิดพลาด</h3>
          <p className="mt-2 text-bliss-600">{error.message}</p>
          <button
            onClick={() => refetch()}
  ...
  if (!hotel) {
    return (
      <div className="flex h-96 items-center justify-center">
        <div className="text-center">
          <Building className="mx-auto h-16 w-16 text-bliss-400" />
          <h3 className="mt-4 text-lg font-semibold text-bliss-900">ไม่พบข้อมูลโรงแรม</h3>
          <Link to="/admin/hotels" className="mt-4 inline-block text-bliss-600 hover:underline">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            ...
            className="rounded-lg p-2 hover:bg-bliss-100"
          >
            <ArrowLeft className="h-5 w-5" />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-bliss-900">{hotel.name_th}</h1>
            <p className="text-bliss-600">{hotel.name_en}</p>
          </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-bliss-600 px-4 py-2 text-white hover:bg-bliss-700"
          >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="rounded-lg bg-white p-6 shadow">
            <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-bliss-900">
              <Building className="h-5 w-5" />
              ข้อมูลพื้นฐาน
            </h2>
            {hotel.description && (
              <p className="mb-4 text-bliss-600">{hotel.description}</p>
            )}
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <p className="text-sm text-bliss-500">ชื่อภาษาไทย</p>
                <p className="font-medium">{hotel.name_th}</p>
              </div>
              <div>
                <p className="text-sm text-bliss-500">ชื่อภาษาอังกฤษ</p>
                <p className="font-medium">{hotel.name_en}</p>
              </div>
              {hotel.website && (
                <div className="md:col-span-2">
                  <p className="text-sm text-bliss-500">เว็บไซต์</p>
                  <a
                    href={hotel.website}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 font-medium text-bliss-600 hover:underline"
                  >
          ...
          <div className="rounded-lg bg-white p-6 shadow">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <User className="h-5 w-5 text-bliss-400" />
                <div>
                  <p className="text-sm text-bliss-500">ผู้ติดต่อ</p>
                  <p className="font-medium">{hotel.contact_person}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="h-5 w-5 text-bliss-400" />
                <div>
                  <p className="text-sm text-bliss-500">เบอร์โทรศัพท์</p>
                  <a href={`tel:${hotel.phone}`} className="font-medium text-bliss-600 hover:underline">
                    {hotel.phone}
                  </a>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="h-5 w-5 text-bliss-400" />
                <div>
                  <p className="text-sm text-bliss-500">อีเมล</p>
                  <a href={`mailto:${hotel.email}`} className="font-medium text-bliss-600 hover:underline">
                    {hotel.email}
                  </a>
                </div>
              </div>
              {(hotel as any).recommended_sales_staff && (
                <div className="flex items-center gap-3">
                  <User className="h-5 w-5 text-bliss-400" />
                  <div>
                    <p className="text-sm text-bliss-500">เซลล์ที่แนะนำ</p>
          ...
          <div className="rounded-lg bg-white p-6 shadow">
            ...
            <p className="mb-4 text-bliss-700">{hotel.address}</p>
            ...
            {hotel.latitude && hotel.longitude ? (
              ...
              <div className="flex h-64 items-center justify-center rounded-lg bg-bliss-100">
                <p className="text-bliss-500">ไม่มีข้อมูลตำแหน่งบนแผนที่</p>
              </div>
            )}

            {hotel.latitude && hotel.longitude && (
              <div className="mt-2 text-sm text-bliss-500">
          ...
          <div className="rounded-lg bg-white p-6 shadow">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              {hotel.tax_id && (
                <div className="md:col-span-2">
                  <p className="text-sm text-bliss-500">เลขประจำตัวผู้เสียภาษี</p>
                  <p className="font-medium font-mono">{hotel.tax_id}</p>
                </div>
              )}
              {!hotel.bank_name && !hotel.bank_account_number && (
                <div className="md:col-span-2">
                  <p className="text-bliss-500">ยังไม่มีข้อมูลการชำระเงิน</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg bg-white p-6 shadow">
            <div className="grid grid-cols-4 gap-4">
              {/* การจอง */}
              <div className="text-center">
                <div className="text-2xl font-bold text-bliss-900">
                  {hotel.total_bookings || 0}
                </div>
                <p className="text-sm text-bliss-500">การจอง</p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-bliss-500">รายได้/เดือน</p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-bliss-500">ส่วนลด</p>
              </div>
            </div>
          </div>
          ...
          {/* Credit Settings */}
          <div className="rounded-lg bg-white p-6 shadow">
            <h3 className="mb-4 font-semibold text-bliss-900 flex items-center gap-2">
              <CreditCard className="h-5 w-5 text-bliss-600" />
              ตั้งค่าเครดิต
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <p className="text-sm text-bliss-500">จำนวนวันเครดิต</p>
                <p className="text-lg font-bold text-bliss-900">{(hotel as any).credit_days || 30} วัน</p>
              </div>
              <div>
                <p className="text-sm text-bliss-500">วันเริ่มรอบเครดิต</p>
              ...
              <div>
                <p className="text-sm text-bliss-500">วันครบรอบในเดือน</p>
                <p className="text-lg font-bold text-bliss-900">
          ...
          {/* Status Management */}
          <div className="rounded-lg bg-white p-6 shadow">
            <h3 className="mb-4 font-semibold text-bliss-900">จัดการสถานะ</h3>
          ...
          {/* Quick Links */}
          <div className="rounded-lg bg-white p-6 shadow">
            <h3 className="mb-4 font-semibold text-bliss-900">ลิงก์ด่วน</h3>
            <div className="space-y-2">
              <Link
                to={`/admin/hotels/${hotel.id}/billing`}
                className="flex items-center gap-2 rounded-lg border px-4 py-2 text-bliss-700 hover:bg-bliss-50"
              >
      ...
      <div className="rounded-lg bg-white p-6 shadow">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-lg font-semibold text-bliss-900">
            <FileText className="h-5 w-5" />
            บิล/ใบแจ้งหนี้ล่าสุด
          </h2>
          <Link
            to={`/admin/hotels/${hotel.id}/billing`}
            className="text-sm text-bliss-600 hover:underline"
          >
            ดูทั้งหมด →
          </Link>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-bliss-50">
              <tr>
                <th className="px-4 py-3 text-left text-sm font-medium text-bliss-700">เลขที่บิล</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-bliss-700">ช่วงเวลา</th>
                <th className="px-4 py-3 text-right text-sm font-medium text-bliss-700">ยอดรวม</th>
                <th className="px-4 py-3 text-right text-sm font-medium text-bliss-700">ส่วนลด</th>
                <th className="px-4 py-3 text-center text-sm font-medium text-bliss-700">สถานะ</th>
                <th className="px-4 py-3 text-center text-sm font-medium text-bliss-700">วันที่จ่าย</th>
                <th className="px-4 py-3 text-center text-sm font-medium text-bliss-700">การกระทำ</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-bliss-200">
              {invoicesLoading ? (
                ...
                invoices.slice(0, 5).map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-bliss-50">
                    <td className="px-4 py-3 text-sm font-medium text-bliss-900">{invoice.bill_number}</td>
                    <td className="px-4 py-3 text-sm text-bliss-600">
                      {new Date(invoice.period_start).toLocaleDateString('th-TH')} - {new Date(invoice.period_end).toLocaleDateString('th-TH')}
                    </td>
                    <td className="px-4 py-3 text-right text-sm font-medium text-bliss-900">
                      ฿{Number(invoice.total_amount || 0).toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-right text-sm text-bliss-600">
                    ...
                    <td className="px-4 py-3 text-center">
                      {invoice.status === 'paid' ? (
                        ...
                        <span className="inline-flex items-center gap-1 rounded-full bg-bliss-100 px-2 py-1 text-xs font-medium text-bliss-700">
                          {invoice.status}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-bliss-600">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-sm text-bliss-600 hover:underline"
                      >
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-bliss-500">
