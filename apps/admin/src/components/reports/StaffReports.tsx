    ...
    return (
      <div className="bg-white rounded-2xl shadow-lg p-8 border border-bliss-100">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-bliss-900 mb-2">ไม่สามารถโหลดข้อมูลพนักงานได้</h2>
          <p className="text-bliss-500 mb-6">กรุณาลองใหม่อีกครั้งหรือติดต่อผู้ดูแลระบบ</p>
          <button
            onClick={() => {
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-bliss-100">
        <h2 className="text-lg font-semibold text-bliss-900 mb-6">ภาพรวมพนักงาน</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-[#565b34] to-[#464a28] rounded-2xl shadow-lg p-6 text-white">
          ...
          <div className="bg-gradient-to-br from-[#c8c29c] to-[#c8c29c] rounded-2xl shadow-lg p-6 text-white">
            <div className="flex items-center justify-between">
              <div>
          ...
          <div className="bg-gradient-to-br from-[#ebe6d0] to-[#dfd9b9] rounded-2xl shadow-lg p-6 text-bliss-800">
            <div className="flex items-center justify-between">
              <div>
              ...
              <Target className="w-12 h-12 text-bliss-700 opacity-80" />
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-bliss-100">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-bliss-900">อันดับพนักงาน</h2>
          <select
        ...
        <div className="space-y-4">
          {isLoading ? (
            Array.from({ length: 5 }).map((_, index) => (
              <div key={index} className="flex items-center gap-4 p-4 bg-bliss-50 rounded-xl animate-pulse">
                <div className="w-12 h-12 bg-bliss-200 rounded-full"></div>
                <div className="flex-1">
                  <div className="w-32 h-4 bg-bliss-200 rounded mb-2"></div>
                  <div className="w-24 h-3 bg-bliss-200 rounded"></div>
                </div>
                <div className="w-16 h-8 bg-bliss-200 rounded"></div>
              </div>
            ))
          ) : (
            (staffRankings.data || []).map((staff) => (
              <div key={staff.staff_id} className="flex items-center gap-4 p-4 bg-bliss-50 rounded-xl hover:bg-bliss-100 transition">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-bliss-900">{staff.name}</h3>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-bliss-100">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-bliss-900">รายละเอียดผลการปฏิบัติงาน</h2>
          <button
            onClick={() => setShowPerformanceDetails(!showPerformanceDetails)}
            className="flex items-center gap-2 text-sm text-bliss-600 hover:text-bliss-700"
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-bliss-200">
                <th className="text-left py-3 px-4 text-sm font-semibold text-bliss-700">ชื่อ</th>
                <th className="text-left py-3 px-4 text-sm font-semibold text-bliss-700">งานสำเร็จ</th>
                <th className="text-left py-3 px-4 text-sm font-semibold text-bliss-700">รายได้</th>
                <th className="text-left py-3 px-4 text-sm font-semibold text-bliss-700">คะแนน</th>
                <th className="text-left py-3 px-4 text-sm font-semibold text-bliss-700">อัตราสำเร็จ</th>
                {showPerformanceDetails && (
                  <>
                    <th className="text-left py-3 px-4 text-sm font-semibold text-bliss-700">ความตรงต่อเวลา</th>
                    <th className="text-left py-3 px-4 text-sm font-semibold text-bliss-700">ความเชี่ยวชาญ</th>
                  </>
                )}
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                Array.from({ length: 5 }).map((_, index) => (
                  <tr key={index} className="border-b border-bliss-100">
                    <td className="py-3 px-4"><div className="w-32 h-4 bg-bliss-200 rounded animate-pulse"></div></td>
                    <td className="py-3 px-4"><div className="w-16 h-4 bg-bliss-200 rounded animate-pulse"></div></td>
                    <td className="py-3 px-4"><div className="w-20 h-4 bg-bliss-200 rounded animate-pulse"></div></td>
                    <td className="py-3 px-4"><div className="w-12 h-4 bg-bliss-200 rounded animate-pulse"></div></td>
                    <td className="py-3 px-4"><div className="w-16 h-4 bg-bliss-200 rounded animate-pulse"></div></td>
                    {showPerformanceDetails && (
                      <>
                        <td className="py-3 px-4"><div className="w-16 h-4 bg-bliss-200 rounded animate-pulse"></div></td>
                        <td className="py-3 px-4"><div className="w-24 h-4 bg-bliss-200 rounded animate-pulse"></div></td>
                      </>
                    )}
                  </tr>
                ))
                ...
                (staffPerformance.data || []).slice(0, 10).map((staff) => (
                  <tr key={staff.staff_id} className="border-b border-bliss-100 hover:bg-bliss-50">
                    <td className="py-3 px-4">
                      <div>
                        <div className="font-medium text-bliss-900">{staff.name}</div>
                        <div className="text-xs text-bliss-500">{staff.email}</div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-bliss-600">{staff.completion_rate}%</span>
                      </div>
                    </td>
                    {showPerformanceDetails && (
                      <>
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <Clock className="w-4 h-4 text-bliss-400" />
                            <span className="text-sm text-bliss-600">{staff.punctuality_score}%</span>
                          </div>
                        </td>
                        <td className="py-3 px-4">
                          <div className="flex flex-wrap gap-1">
                            {staff.specializations.slice(0, 2).map((spec) => (
                              <span key={spec} className="px-2 py-1 bg-bliss-100 text-bliss-800 text-xs rounded-full">
                                {spec}
                              </span>
                            ))}
                            {staff.specializations.length > 2 && (
                              <span className="px-2 py-1 bg-bliss-100 text-bliss-600 text-xs rounded-full">
                                +{staff.specializations.length - 2}
                              </span>
                            )}
                          </div>
              ...
              {!isLoading && (!staffPerformance.data || staffPerformance.data.length === 0) && (
                <tr>
                  <td colSpan={showPerformanceDetails ? 7 : 5} className="py-8 text-center text-bliss-500">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-bliss-100">
        <h2 className="text-lg font-semibold text-bliss-900 mb-6">สรุปรายได้พนักงาน</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="p-4 bg-bliss-50 rounded-xl border border-bliss-100">
                <div className="w-24 h-6 bg-bliss-200 rounded mb-3 animate-pulse"></div>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <div className="w-16 h-4 bg-bliss-200 rounded animate-pulse"></div>
                    <div className="w-12 h-4 bg-bliss-200 rounded animate-pulse"></div>
                  </div>
                  <div className="flex justify-between">
                    <div className="w-12 h-4 bg-bliss-200 rounded animate-pulse"></div>
                    <div className="w-16 h-4 bg-bliss-200 rounded animate-pulse"></div>
            ...
            (staffEarnings.data || []).slice(0, 6).map((staff) => (
              <div key={staff.staff_id} className="p-4 bg-bliss-50 rounded-xl border border-bliss-100">
                <h3 className="font-semibold text-bliss-900 mb-3">{staff.name}</h3>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-bliss-500">รายได้:</span>
