    ...
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 99999 }}>
      {!show ? (
        ...
        <div style={{ background: '#1e293b', color: '#e2e8f0', padding: '8px', maxHeight: '50vh', overflow: 'auto', fontSize: '10px', fontFamily: 'monospace' }}>
          ...
          <button onClick={() => { const text = 'saved_path: ' + (savedPath || 'NULL') + '\ncurrent_url: ' + window.location.href + '\n' + logs.map((l: any) => new Date(l.t).toLocaleTimeString() + ' ' + l.step + ' ' + (l.data ? JSON.stringify(l.data) : '') + '\n' + l.url).join('\n'); navigator.clipboard.writeText(text).then(() => alert('Copied!')).catch(() => { const ta = document.createElement('textarea'); ta.value = text; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta); alert('Copied!') }) }} style={{ float: 'right', background: '#3b82f6', color: '#fff', border: 'none', padding: '2px 8px', borderRadius: '4px', fontSize: '10px', marginRight: '4px' }}>Copy</button>
