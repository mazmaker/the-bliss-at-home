...
import { Clock, Wallet, Calendar, TrendingUp } from 'lucide-react';
...
export function ExtensionInfo({
  ...
  return (
    <div className={`bg-bliss-50 border border-bliss-200 rounded-lg p-4 ${className}`}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="text-center p-3 bg-white rounded-lg border border-bliss-100">
          <div className="flex items-center justify-center gap-1 mb-1">
            <Clock className="w-4 h-4 text-bliss-600" />
            <span className="text-sm text-bliss-600">เวลาเพิ่ม</span>
          </div>
        ...
        <div className="text-center p-3 bg-white rounded-lg border border-bliss-100">
          <div className="flex items-center justify-center gap-1 mb-1">
            <Wallet className="w-4 h-4 text-bliss-600" />
            <span className="text-sm text-bliss-600">รายได้เพิ่ม</span>
          </div>
      ...
      {/* Extension History */}
      <div>
        <h4 className="font-medium text-bliss-700 mb-2 flex items-center gap-1">
        ...
        <div className="space-y-2">
          {extensions
            ...
            .map((extension, index) => (
              <div
                key={extension.id}
                className="flex items-center justify-between p-2 bg-white rounded border border-bliss-100 text-sm"
              >
                ...
                <div className="flex items-center gap-3 text-right">
                  <span className="text-bliss-700 font-medium">+{extension.duration} นาที</span>
      ...
      {/* Note */}
      <div className="mt-3 p-2 bg-bliss-100 rounded text-xs text-bliss-700">
        {useFixedRate
