-- ============================================
-- Seed Mockup Jobs for Staff App Testing
-- ============================================
-- วิธีใช้: คัดลอก SQL ทั้งหมดนี้ไปวางใน Supabase Studio > SQL Editor และกด RUN
-- ============================================
...
DECLARE
    ...
    v_staff_id UUID;
BEGIN
    -- หา Staff ID (ผู้ใช้งานที่ login อยู่ปัจจุบัน)
    SELECT id INTO v_staff_id FROM profiles WHERE role = 'STAFF' LIMIT 1;

    IF v_staff_id IS NULL THEN
        RAISE EXCEPTION 'ไม่พบ Staff profile กรุณาสร้าง Staff account ก่อน';
    END IF;
    ...
    -- ลบงานทดสอบเก่า (ถ้ามี)
    DELETE FROM jobs WHERE customer_id = v_customer_id;
    ...
    RAISE NOTICE 'รวม: % งาน', (SELECT COUNT(*) FROM jobs WHERE customer_id = v_customer_id);
    RAISE NOTICE '- Pending: % งาน', (SELECT COUNT(*) FROM jobs WHERE customer_id = v_customer_id AND status = 'pending');
    RAISE NOTICE '- Assigned/Confirmed: % งาน', (SELECT COUNT(*) FROM jobs WHERE customer_id = v_customer_id AND status IN ('assigned', 'confirmed'));
    RAISE NOTICE '- Completed: % งาน', (SELECT COUNT(*) FROM jobs WHERE customer_id = v_customer_id AND status = 'completed');
    RAISE NOTICE '============================================';
...
FROM jobs
WHERE customer_id IN (SELECT id FROM profiles WHERE email = 'customer.mockup@theblissathome.com')
GROUP BY status
ORDER BY
    CASE status
        WHEN 'pending' THEN 1
