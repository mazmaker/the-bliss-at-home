...
-- ============================================
-- 0. Drop existing tables if they exist (for clean recreation)
-- ============================================
DROP VIEW IF EXISTS staff_earnings_summary;
...
CREATE INDEX idx_bank_accounts_is_primary ON bank_accounts(staff_id, is_primary);
...
CREATE INDEX idx_payouts_staff_id ON payouts(staff_id);
CREATE INDEX idx_payouts_status ON payouts(status);
CREATE INDEX idx_payouts_period ON payouts(period_start, period_end);
CREATE INDEX idx_payouts_staff_status ON payouts(staff_id, status);
...
CREATE INDEX idx_payout_jobs_payout_id ON payout_jobs(payout_id);
CREATE INDEX idx_payout_jobs_job_id ON payout_jobs(job_id);
...
DROP POLICY IF EXISTS "Staff can view their bank accounts" ON bank_accounts;
DROP POLICY IF EXISTS "Staff can insert bank accounts" ON bank_accounts;
DROP POLICY IF EXISTS "Staff can update their bank accounts" ON bank_accounts;
DROP POLICY IF EXISTS "Staff can delete their bank accounts" ON bank_accounts;
...
-- Drop existing policies first (idempotent)
DROP POLICY IF EXISTS "Staff can view their payouts" ON payouts;
...
-- Drop existing policies first (idempotent)
DROP POLICY IF EXISTS "Staff can view their payout jobs" ON payout_jobs;
