-- Alternative approach: Create table with TEXT type first, then convert to enum
-- Use this file ONLY if the main migration file still fails
...
-- Create staff_documents table with TEXT columns first
CREATE TABLE IF NOT EXISTS staff_documents (
    -- Primary identification
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    staff_id UUID NOT NULL REFERENCES staff(id) ON DELETE CASCADE,

    -- Document information (using TEXT instead of enum initially)
    ...
    -- Status tracking (using TEXT instead of enum initially)
...
-- Now convert TEXT columns to enum types
-- Use parentheses to explicitly reference the column value, not the type
ALTER TABLE staff_documents
    ALTER COLUMN document_type TYPE document_type USING (document_type)::document_type;

ALTER TABLE staff_documents
    ALTER COLUMN verification_status TYPE document_status USING (verification_status)::document_status;
