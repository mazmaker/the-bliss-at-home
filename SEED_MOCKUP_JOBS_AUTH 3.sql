...
-- ใช้ Staff ID ของ User ที่ Login อยู่ปัจจุบัน
-- ============================================
...
BEGIN
    ...
    IF v_staff_id IS NULL THEN
        RAISE EXCEPTION 'กรุณา Login ก่อนรัน SQL นี้';
    END IF;
    ...
    (
        ...
        'โรงแลม Four Seasons Bangkok', 13.7234586, 100.5403421, 4.8,
...
FROM jobs
WHERE staff_id = auth.uid() OR (customer_id IN (SELECT id FROM profiles WHERE email = 'customer.mockup@theblissathome.com') AND status = 'pending')
GROUP BY status
ORDER BY
    CASE status
        WHEN 'pending' THEN 1
...
WHERE scheduled_date = CURRENT_DATE
  AND (staff_id = auth.uid() OR (customer_id IN (SELECT id FROM profiles WHERE email = 'customer.mockup@theblissathome.com') AND status = 'pending'))
