-- ========================================
-- Extension Acknowledgment System
-- Date: 2026-03-25
...
ON extension_acknowledgments(staff_id, acknowledged_at)
...
USING (
  EXISTS (
    ...
    WHERE s.id = extension_acknowledgments.staff_id
...
COMMENT ON COLUMN extension_acknowledgments.staff_id IS 'Staff who needs to acknowledge';
...
BEGIN
  ...
  IF NEW.is_extension = TRUE THEN
    ...
    RAISE NOTICE 'Created extension acknowledgment for booking_service %, staff %',
...
BEGIN
  ...
  WHERE s.profile_id = staff_profile_id
...
BEGIN
  ...
  AND j.staff_id IS NOT NULL
  ON CONFLICT (booking_service_id, staff_id) DO NOTHING;
